...
import { getProKey, clearProKey } from '../../utils/storage';
...
async function callOpenAI(apiKey, userPrompt, context) {
  const system = `Olet Lukkari.io:n älykäs kurssisuosittelija. Autat suomalaisen lukion opiskelijaa suunnittelemaan kurssivalintojaan optimaalisesti. Vastaa aina suomeksi. Ole ytimekäs, kannustava ja konkreettinen. Käytä lyhyitä kappaleita. Voit käyttää listoja selkeyttämään vastauksia.`;
  const userMsg = context
    ? `${context}\n\nKysymykseni: ${userPrompt}`
    : `Opiskelijalla ei ole vielä kurssivalintoja tallennettuna ilmaisversiossa.\n\nKysymykseni: ${userPrompt}`;
  ...
  if (!res.ok) {
    ...
    throw new Error(err?.error?.message || `Virhe ${res.status}`);
  }
    ...
    return (
      <div style={{
        ...
        <p style={{ fontSize: 12, color: "#605c58", textAlign: "center" }}>
          Ei kursseja tallennettuna — lisää kursseja{" "}
          <button
            onClick={() => { history.replaceState(null, "", window.location.pathname); window.dispatchEvent(new HashChangeEvent("hashchange")); }}
    ...
    <div style={{
      ...
      <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 4 }}>
        {Array.from({ length: school.periodCount }, (_, pi) => {
          const periodCourses = Array.from({ length: school.palkkiCount }, (_, bi) => {
            const v = (selections || {})[`p${pi + 1}-b${bi + 1}`];
            return v?.trim() ? { label: v.trim(), bi } : null;
          }).filter(Boolean);
          if (!periodCourses.length) return null;
          return (
            <div key={pi} style={{ flexShrink: 0, minWidth: 120 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {periodCourses.map(({ label, bi }, idx) => {
      ...
      <p style={{ fontSize: 10, color: "#605c58", marginTop: 12 }}>
        {filled.length} kurssia · {school.periodCount} periodia · {school.palkkiCount} palkkia
      </p>
    </div>
  );
}
...
function SpinnerIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
      style={{ animation: "orb-rotate 0.9s linear infinite" }}>
...
export function ProApp() {
  ...
  useEffect(() => {
    ...
    if (!getProKey()) { window.location.hash = "/pro-login"; return; }
    const s = loadState();
    setSchedule(s || {});
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    const key = getProKey();
    if (!key) { window.location.hash = "/pro-login"; return; }
    setAiLoading(true);
    setAiError(null);
    ...
    try {
      ...
      const text = await callOpenAI(key, q, context);
      setHasResponse(true);
  ...
  const useSuggested = useCallback((s) => {
    setPrompt(s);
  }, []);
    ...
    <div style={{ minHeight: "100dvh", background: "#08080f", position: "relative" }}>
      ...
      <video autoPlay loop muted playsInline
        style={{
          ...
          objectFit: "cover", opacity: 0.22, zIndex: 0, pointerEvents: "none",
        }}
      ...
      <header style={{
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={goToFree} style={{
            fontSize: 11, fontWeight: 500, color: "#605c58",
      ...
      {/* Main content */}
      <main style={{
        maxWidth: 860, margin: "0 auto", padding: "28px 16px 80px",
        position: "relative", zIndex: 1,
      }}>
        {/* Page title */}
        <div style={{ marginBottom: 22 }}>
          <h1 className="fr" style={{ fontSize: 34, fontWeight: 500, letterSpacing: "-0.025em", color: "#f0ede8", lineHeight: 1.1, marginBottom: 6 }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
            <div style={{
              ...
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="rgba(200,180,255,0.9)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 14 }}>
            {SUGGESTED.map((s, i) => (
              <button key={i} onClick={() => useSuggested(s)} style={{
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
            <textarea
              ...
              style={{
                ...
                lineHeight: 1.5, transition: "border-color 0.14s",
              }}
          ...
          {(hasResponse || aiLoading || aiError) && (
            <div style={{
              ...
              {aiError ? (
                <p style={{
                  fontSize: 12, color: "oklch(0.72 0.14 20)",
                  background: "rgba(255,80,60,0.08)", border: "1px solid rgba(255,80,60,0.16)",
                ...
                <div ref={responseRef} style={{
                  ...
                  {aiLoading && !displayedText && (
                    <span style={{ color: "#605c58" }}>
                      <SpinnerIcon/>{" "}Ajatellaan…
                    </span>
                  )}
                  {/* Blinking cursor while typing */}
                  {hasResponse && displayedText && displayedText.length < 9999 && (
                    <span style={{
        ...
        {/* Footer note */}
        <p style={{ fontSize: 10, color: "rgba(255,255,255,0.12)", textAlign: "center", marginTop: 32 }}>
          Vastauksesi generoidaan OpenAI:n gpt-4o-mini-mallilla omalla API-avaimellasi · Tietoja ei lähetetä Lukkari.io:n palvelimille
        </p>
      </main>
    </div>
  );
